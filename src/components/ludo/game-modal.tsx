...
export function LudoGameModal({ isOpen, onClose }: GameModalProps) {
  ...
  const handleCreateGame = () => {
    if (mode === 'live') {
      ...
      navigate(`/game/ludo?mode=live&type=create&stake=${stake}`);
    } else {
      toast.success('Game created successfully! Waiting for opponent...');
      navigate(`/game/ludo?mode=demo&type=create&stake=10`);
    }
    onClose();
  };
  ...
  const handleJoinGame = (game: AvailableGame) => {
    if (mode === 'live') {
      ...
      navigate(`/game/ludo?mode=live&type=join&stake=${game.stake}`);
    } else {
      toast.success('Joining game...');
      navigate(`/game/ludo?mode=demo&type=join&stake=${game.stake}`);
    }
    onClose();
  };
  ...
  const handleInviteFriend = (friend: Friend) => {
    if (mode === 'live') {
      ...
      navigate(`/game/ludo?mode=live&type=invite&stake=${stake}`);
    } else {
      toast.success(`Invitation sent to ${friend.username}`);
      navigate(`/game/ludo?mode=demo&type=invite&stake=10`);
    }
    onClose();
  };
  ...
  const handleComputerGame = () => {
    if (mode === 'live') {
      ...
      navigate(`/game/ludo?mode=live&type=computer&stake=${stake}`);
    } else {
      toast.success('Starting game against computer...');
      navigate(`/game/ludo?mode=demo&type=computer&stake=10`);
    }
    onClose();
  };
